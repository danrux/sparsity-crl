...
from generate_balls_dataset import generate_missing_ball_dataset as generate_ball_dataset
...
def main():
    ...
    class Constrained_DE(cooper.ConstrainedMinimizationProblem):
        def __init__(self,latent_dim=args.n_balls, n_classes=len(args.masks)):
        ...
        def compute_loss(self, x, x_hat, mu):
            ...
            stat_loss = 0
            log_var_prior = torch.tensor(np.array(args.masks)[:,:,0].reshape((len(args.masks),-1))).to(device)
            log_var_prior[log_var_prior==0]=-20
            mu_prior = torch.ones(args.n_balls).to(device)*2
            #mu_prior = torch.zeros(args.n_balls).to(device)+10
        ...
        def closure(self,  inputs):
            ...
            ineq_defect = torch.sum(torch.abs(z_hat))/args.group_batch_size/args.n_balls/len(args.masks) - args.sparse_level
    
            return cooper.CMPState(loss=loss1+stat_loss, ineq_defect=ineq_defect, eq_defect=None)
            
    
    g = encoders.ResNetEnc(resnet18, hidden_size=200, encoding_size=args.n_balls, output_normalization="bn")
    f_hat = encoders.ResNetDec(z_dim=args.n_balls)
    ...
    while (
        ...
        if global_step % args.n_log_steps == 1 or global_step == args.n_steps:
                ...
                mcc, cor_m = MCC(z_disentanglement, hz_disentanglement, args.n_balls)
                   
                    
                mcc = mcc/args.n_balls
...
def parse_args():
    ...
    parser.add_argument("--masks", type=int, default=[[[1,0],[0,0]], [[0,0],[1,0]]])  # 0 is masked variables
    parser.add_argument("--mask-value", type=int, default=[[0.0,0.3],[0.0,0.6]])
    parser.add_argument("--Sigma", type=float, default=[[0.01,0.00],[0.00,0.01]])
    parser.add_argument("--evaluate", action='store_true')
    parser.add_argument("--seed", type=int, default=222)
    parser.add_argument("--model-dir", type=str, default="Balls_missing")
